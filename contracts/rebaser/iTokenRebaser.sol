...
contract iTokenRebaser {
     ...
     * @notice Sets the reserve contract
     */
    event TreasuryIncreased(uint256 reservesAdded, uint256 itokensSold, uint256 itokensFromReserves, uint256 itokensToReserves);
    ...
    uint256 public constant rebaseDelay = 1 days;

    /// @notice Time of TWAP initialization
    uint256 public timeOfTWAPInit;

    /// @notice iToken token address
    ...
    /// @notice pair for reserveToken <> iToken
    ...
    uint256 public publicGoodsPerc;
    ...
    /// @notice Whether or not this token is first in uniswap iToken<>Reserve pair
    ...
    constructor(
        address itokenAddress_,
        address reserveToken_,
        address uniswap_factory,
        address reservesContract_,
        address publicGoods_,
    ...
    public
    {
        ...
        if (token0 == itokenAddress_) {
            isToken0 = true;
        } else {
            isToken0 = false;
        }
        // uniswap iToken<>Reserve pair
        ...
        itokenAddress = itokenAddress_;

        publicGoods = publicGoods_;
    ...
    */
    ...
    /**
    @notice Updates public goods address
    @param publicGoods_ the new public goods address
    *
    */
    function setPublicGoods(address publicGoods_) public onlyGov
    {
        publicGoods = publicGoods_;
    }

    /**
    @notice Update percent of treasury to public goods address
    @param publicGoodsPerc_ the new percent of treasury to public goods address
    *
    */
    function setPublicGoodsPerc(uint256 publicGoodsPerc_) public onlyGov
    {
        publicGoodsPerc = publicGoodsPerc_;
    }
    ...
    */
    ...
    public
    {
        ...
        iElasticTokenInterface itoken = iElasticTokenInterface(itokenAddress);

        if (positive) {
            require(itoken.itokensScalingFactor().mul(BASE.add(indexDelta)).div(BASE) < itoken.maxScalingFactor(), "new scaling factor will be too big");
        }


        uint256 currSupply = itoken.totalSupply();
        ...
        // rebase
        // ignore returned var
        itoken.rebase(epoch, indexDelta, positive);
    ...
    public
    {
        ...
        if (uniVars.amountFromReserves > 0) {
            ...
            itoken.transferFrom(reservesContract, uniswap_pair, uniVars.amountFromReserves);
            if (uniVars.amountFromReserves < uniVars.itokensToUni) {
                // if the amount from reserves > itokensToUni, we have fully paid for the yCRV tokens
                // thus this number would be 0 so no need to mint
                itoken.mint(uniswap_pair, uniVars.itokensToUni.sub(uniVars.amountFromReserves));
            }
        } else {
            // mint to uniswap
            itoken.mint(uniswap_pair, uniVars.itokensToUni);
        }

        // mint unsold to mintAmount
        if (uniVars.mintToReserves > 0) {
            itoken.mint(reservesContract, uniVars.mintToReserves);
        }

        // transfer reserve token to reserves
        if (isToken0) {
            if (publicGoods != address(0) && publicGoodsPerc > 0) {
                uint256 amountToPublicGoods = amount1.mul(publicGoodsPerc).div(BASE);
                SafeERC20.safeTransfer(IERC20(reserveToken), reservesContract, amount1.sub(amountToPublicGoods));
                SafeERC20.safeTransfer(IERC20(reserveToken), publicGoods, amountToPublicGoods);
                emit TreasuryIncreased(amount1.sub(amountToPublicGoods), uniVars.itokensToUni, uniVars.amountFromReserves, uniVars.mintToReserves);
            } else {
                SafeERC20.safeTransfer(IERC20(reserveToken), reservesContract, amount1);
                emit TreasuryIncreased(amount1, uniVars.itokensToUni, uniVars.amountFromReserves, uniVars.mintToReserves);
            }
        } else {
            if (publicGoods != address(0) && publicGoodsPerc > 0) {
                uint256 amountToPublicGoods = amount0.mul(publicGoodsPerc).div(BASE);
                SafeERC20.safeTransfer(IERC20(reserveToken), reservesContract, amount0.sub(amountToPublicGoods));
                SafeERC20.safeTransfer(IERC20(reserveToken), publicGoods, amountToPublicGoods);
                emit TreasuryIncreased(amount0.sub(amountToPublicGoods), uniVars.itokensToUni, uniVars.amountFromReserves, uniVars.mintToReserves);
            } else {
                SafeERC20.safeTransfer(IERC20(reserveToken), reservesContract, amount0);
                emit TreasuryIncreased(amount0, uniVars.itokensToUni, uniVars.amountFromReserves, uniVars.mintToReserves);
            }
        }
    }
    ...
    internal
    {
        ...
        // check if protocol has excess itoken in the reserve
        uint256 excess = itoken.balanceOf(reservesContract);


        uint256 tokens_to_max_slippage = uniswapMaxSlippage(token0Reserves, token1Reserves, offPegPerc);

        UniVars memory uniVars = UniVars({
            itokensToUni : tokens_to_max_slippage, // how many itokens uniswap needs
            amountFromReserves : excess, // how much of itokensToUni comes from reserves
            mintToReserves : 0 // how much itokens protocol mints to reserves
            });
        ...
        if (isToken0) {
            if (tokens_to_max_slippage > mintAmount.add(excess)) {
                ...
                uniVars.itokensToUni = mintAmount + excess;
                ...
                if (tokens_to_max_slippage > excess) {
                    ...
                    // swap up to slippage limit, taking entire itoken reserves, and minting part of total
    ...
    internal
    {
        ...
        for (uint256 i = 0; i < balGulpPairs.length; i++) {
            BAL(balGulpPairs[i]).gulp(itokenAddress);
        }
